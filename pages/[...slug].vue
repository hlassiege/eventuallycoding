...
<!--                            {{ article.readTime.text }}-->
          ...
          Partager sur :
                    ...
                    <div class="mt-4">
                        ...
                        <a :href="'https://facebook.com/sharer/sharer.php?u='+ postLink" target="_blank" rel="noopener" aria-label="">
                        ...
                        <a :href="'https://twitter.com/share?url=' + postLink + '&text=' + article.title +'&via='+ siteMetadata.twitter_user" target="_blank" title="Share on Twitter" rel="noopener noreferrer">
                ...
                <div class="pt-16" v-if="relatedArticles">
...
const postLink = ref(siteMetaInfo.siteUrl  + path);

const {data} = await useAsyncData(`content-${path}`, async () => {
    const article = await queryContent("articles").where({_path: `/articles${path}`}).find()
    return article[0] ? article[0] : {}
});

const article = ref(data.value);

const {data : recommendedArticles} = await useAsyncData(`relatedArticles-${path}`, async () => {
        ...
        for (let i = 0; i < article.value.tags.length; i++) {
            ...
            relatedArticles.forEach(relatedArticle => {
                var isAlreadyIn = allRelatedArticles.find(function (element) {
                    return element.id === relatedArticle.id;
                });
                if (!isAlreadyIn) {
                    allRelatedArticles.push(relatedArticle);
                }
            });
        }
    }
    ...
    return shuffledRelatedArticles;
});

const relatedArticles = ref(recommendedArticles.value);
...
useHead({
            ...
            meta: [
                ...
                { name: "article:published_time", content: article.value.date  },
                // { name: "article:article:modified_time", content: article.value.updatedAt  },
