<template>
  <div class="mb-6 text-center md:w-3/4 w-full mx-auto">
    <h2 class="mt-6 text-4xl text-gray-700 font-bold capitalize">
      Salut !
    </h2>
    <p class="mt-8 mb-4 text-center text-xl">
      Je m’appelle Hugo. Je suis développeur depuis 20 ans, CTO et co-fondateur de <a class="text-slate-500 hover:text-slate-600" href="https://www.malt.fr">Malt</a> depuis 10 ans.
    <br/>
      Je travaille depuis le crack de la bulle Internet et le bug de l’an 2000.
    <br/>
      Pourtant, personne n’a encore fait le lien et je cours toujours alors chuutt…
    </p>


  </div>
</template>
