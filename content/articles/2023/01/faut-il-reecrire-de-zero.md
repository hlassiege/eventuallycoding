...
Eh bien, c'est ce que je vous propose de voir dans ce billet. On va parler de l'évolution de la stack technique de Malt depuis 2012, de gestion de la dette et d'une approche de la réécriture partielle et itérative.
...
On se marche un tout petit peu dessus de temps en temps et on conclut que ce serait intéressant de pouvoir releaser des parties du site indépendamment. Cela permettait par exemple de sortir le moteur de recherche tandis qu'on continuait à avoir le moteur de paiement en draft.
...
Mais la migration sur OVH, notamment pour des raisons de coût des addons était plutôt pragmatique et nous a apporté une souplesse très pratique à cette période.
...
Suite à 2018, nous avons rencontré de nombreuses limitations sur le cloud OVH. A l'époque l'api OpenStack n'était pas stable et nous rencontrions de nombreuses erreurs 500. Il devenait de plus en plus évident qu'il fallait construire des solutions complexes pour résoudre des sujets, sans que ce soit notre cœur de métier (la gestion des clés privée, la gestion des logs, la réplication mongo vers PG ou la création d'un datawarehouse par exemple).
Nous avons donc décidé d'aller sur GCP en 2019. Plus exactement nous tournons désormais sur GKE (Kubernetes managé). Traefik a définitivement remplacé Nginx en tant que reverse proxy grâce à son intégration native avec Docker/Kube.
...
- parfois on doit faire défaut sur une dette. Ce défaut se traduit par une réécriture complète de l'application avec le cout associé. Dans le pire des cas, c'est la fin du produit et de l'entreprise.
...
Les choix effectués sur Malt ont tous été fait en fonction du contexte.
Les standards de développement ont toujours été adaptés à la maturité de l'entreprise.
...
Et c'est l'un de ces projets dont je souhaite parler prochainement, la modernisation complète de notre stack frontend, comment passer de 5 technos frontend à uniquement une seule, et comment se débarrasser totalement de Java côté frontend (les fameuses JSP). 
Et de façon un peu poétique, ce projet s'intitule Singapore car c'est l'histoire d'un voyage en dehors de l'île de Java.
